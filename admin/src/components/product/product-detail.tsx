    ...
    <div className="container mx-auto p-6 space-y-8">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <Button variant="outline" onClick={() => router.push(`/products/edit-product/${product.id}`)}>
...
function VariantRow({ variant, onUpdate }: VariantRowProps) {
  ...
  const saveBasic = async () => {
    setUpdating(true);
    try {
      const response = await fetch(`${API_URL}/variants/${variant.id}`, {
        method: "PATCH",
  ...
  const saveOptions = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        toast.error('Failed to update variant options');
      }
    } catch {
      toast.error('Error updating variant options');
    } finally {
      setUpdating(false);
    }
  };
