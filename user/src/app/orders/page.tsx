      ...
      <main className="flex-1 py-10 md:py-16">
        <div className="sj-container space-y-8">
          ...
          <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 pb-6 border-b border-slate-200/80">
            ...
            <Link
              href="/product"
              className="inline-flex items-center gap-2 text-xs font-bold uppercase text-slate-600 hover:text-[#DF9F28] tracking-wider"
            >
          ...
          <div className="flex flex-col sm:flex-row items-center justify-between gap-4 bg-white p-4 rounded-2xl border border-slate-200 shadow-xs">
            ...
            <div className="relative w-full sm:w-72">
              <input
                ...
                className="w-full pl-9 pr-4 py-2 bg-slate-50 border border-slate-200 rounded-xl text-xs text-slate-900 placeholder:text-slate-400 focus:outline-none focus:border-[#DF9F28]"
              />
          ...
          {filteredOrders.length === 0 ? (
            <div className="max-w-md mx-auto py-16 bg-white rounded-3xl border border-slate-200 text-center space-y-4 p-8">
              ...
              <h3 className="text-lg font-bold text-slate-900 font-sans">No orders found</h3>
              <p className="text-xs text-slate-500">
                You have not placed any orders matching this criteria yet.
              </p>
              <Link
                href="/product"
                className="inline-block px-6 py-2.5 bg-[#DF9F28] hover:bg-[#C6891E] text-slate-950 text-xs font-black uppercase rounded-full tracking-wider shadow-sm cursor-pointer"
              >
                Start Shopping
              </Link>
            </div>
                ...
                return (
                  ...
                  >
                    ...
                    <div
                      ...
                      className="p-5 sm:p-6 bg-slate-50/70 border-b border-slate-100 flex flex-col md:flex-row md:items-center justify-between gap-4 cursor-pointer hover:bg-slate-50 transition-colors"
                    >
                      <div className="flex flex-wrap items-center gap-4 sm:gap-6">
                        <div>
                          <p className="text-[11px] font-bold uppercase tracking-wider text-slate-400">Order ID</p>
                          <p className="text-sm font-bold font-mono text-slate-900">{order.id}</p>
                        </div>
                        <div>
                          <p className="text-[11px] font-bold uppercase tracking-wider text-slate-400">Placed On</p>
                      ...
                      {order.items.map((item, idx) => (
                        <div key={idx} className="flex items-center justify-between gap-4 py-2">
                          <div className="flex items-center gap-4">
                            <div className="relative w-16 h-20 rounded-xl overflow-hidden bg-slate-100 shrink-0 border border-slate-100">
                            ...
                            <div>
                              ...
                              <p className="text-[11px] text-slate-400 font-medium">{item.category} • Size: {item.size} • Qty: {item.quantity}</p>
                              <p className="text-xs font-semibold text-slate-800 mt-1">₹{item.price.toLocaleString('en-IN')}</p>
                            </div>
                          </div>

                          <button
                            onClick={(e) => handleBuyAgain(e, item)}
                            className="px-4 py-2 bg-slate-100 hover:bg-slate-900 hover:text-white text-slate-800 rounded-xl text-xs font-semibold uppercase tracking-wider transition-colors shrink-0"
                          >
                            Buy Again
                          </button>
                        </div>
                    ...
                    {isExpanded && (
                      <div className="p-5 sm:p-6 bg-slate-50 border-t border-slate-100 grid grid-cols-1 sm:grid-cols-2 gap-6 text-xs">
                        <div className="space-y-2">
                          <h5 className="font-bold text-slate-900 uppercase tracking-wider flex items-center gap-1.5">
                        ...
                        <div className="space-y-2">
                          ...
                          {order.status === 'PENDING' && (
                            <button
                              ...
                              className="text-xs text-rose-600 hover:underline font-semibold pt-2 block"
                            >
                              Cancel Order
                            </button>
                          )}
          ...
          {orderToCancel && (
            <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-xs">
              <div className="bg-white rounded-3xl p-6 sm:p-8 max-w-sm w-full space-y-4 shadow-2xl border border-slate-200">
                ...
                <h3 className="text-base font-bold text-slate-900 text-center">Cancel Order?</h3>
                <p className="text-xs text-slate-500 text-center">
                  Are you sure you want to cancel order <span className="font-mono font-bold text-slate-800">{orderToCancel}</span>? This action cannot be undone.
                </p>
                <div className="flex gap-3 pt-2">
                  <button
                    onClick={handleConfirmCancel}
                    className="flex-1 py-2.5 bg-rose-600 text-white text-xs font-bold uppercase tracking-wider rounded-xl hover:bg-rose-500"
                  >
                    Confirm Cancel
                  </button>
                  <button
