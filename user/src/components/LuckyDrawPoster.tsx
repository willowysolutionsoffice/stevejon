...
export default function LuckyDrawPoster() {
  ...
  return (
    <section className="sj-container">
      <div className="relative rounded-3xl overflow-hidden bg-[#111111] text-white border border-zinc-800 shadow-md">
        <div className="grid grid-cols-1 lg:grid-cols-12 items-center">
          ...
          <div className="lg:col-span-7 p-6 sm:p-10 lg:p-12 space-y-5">
            ...
            <div className="space-y-2 pt-2">
              ...
              <div className="grid grid-cols-4 gap-2 max-w-xs text-center font-mono">
                <div className="bg-zinc-900/90 border border-zinc-800 rounded-xl p-2.5">
                  <span className="block text-lg sm:text-xl font-bold text-white leading-none">{pad(timeLeft.days)}</span>
                  <span className="text-[10px] text-zinc-400 font-sans font-medium uppercase mt-1 block">Days</span>
                </div>
                <div className="bg-zinc-900/90 border border-zinc-800 rounded-xl p-2.5">
                  <span className="block text-lg sm:text-xl font-bold text-white leading-none">{pad(timeLeft.hours)}</span>
                  <span className="text-[10px] text-zinc-400 font-sans font-medium uppercase mt-1 block">Hours</span>
                </div>
                <div className="bg-zinc-900/90 border border-zinc-800 rounded-xl p-2.5">
                  <span className="block text-lg sm:text-xl font-bold text-white leading-none">{pad(timeLeft.minutes)}</span>
                  <span className="text-[10px] text-zinc-400 font-sans font-medium uppercase mt-1 block">Mins</span>
                </div>
                <div className="bg-zinc-900/90 border border-zinc-800 rounded-xl p-2.5">
                  <span className="block text-lg sm:text-xl font-bold text-[#DF9F28] leading-none">{pad(timeLeft.seconds)}</span>
                  <span className="text-[10px] text-zinc-400 font-sans font-medium uppercase mt-1 block">Secs</span>
                </div>
              </div>
            </div>
            ...
            <div className="pt-2">
              <Link
                href="/lucky-draw"
                className="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-[#DF9F28] hover:bg-[#C6891E] text-zinc-950 font-semibold text-xs sm:text-sm tracking-wide transition-all shadow-sm active:scale-98 focus-visible:ring-2 focus-visible:ring-amber-400"
              >
          ...
          <div className="lg:col-span-5 relative aspect-square lg:aspect-auto min-h-[320px] lg:h-full bg-zinc-900 border-t lg:border-t-0 lg:border-l border-zinc-800">
            <Image
            ...
            />
            <div className="absolute inset-x-4 bottom-4 p-3.5 rounded-xl bg-zinc-950/90 backdrop-blur-md border border-zinc-800 text-white">
